...
\end{equation*}
to hold for Riemann integral. \yellow{Relationship with Riemann Integral} We'll talk about
\begin{enumerate}
...
A measure space is a \key{probability space} if \(\mu =1\), we write it as \((\Omega, \mathcal{F}, P)\). A random variable/vector is a measurable function on probability space \(X: \Omega \to \mathbb{R}/\mathbb{R}^{d}\) with the Borel \(\sigma\)-algebra.

\subsubsection{Distribution and Density}

\key{Distribution} is the probability induced by a random variable \(X\), such that \(\mu(A) = P\pqty{X \in A}\). \key{Density} is defined via some methods(\yellow{To be added}). Its use is:
\begin{equation*}
    \int g(X) \dd{P} = \int g(x)f(x)\dd{x}
\end{equation*}

\subsubsection{Check for Measurability of Random Variables}

For random variables \(X_{n}, Y_{n}\), \(X_{n} +Y_{n}\) and \(\sup X_{n}\) are measurable. 
